    ...
    for slide_idx, slide in enumerate(prs.slides):
        ...
        try:
            if slide.has_notes_slide and slide.notes_slide.notes_text_frame:
                ...
                if notes_text:
                    # Create a special ShapeData-like entry for notes
                    # Use a stable key "notes"
                    slide_data["notes"] = notes_text
        except (AttributeError, Exception):
            pass

        inventory[f"slide-{slide_idx}"] = slide_data
...
def get_inventory_as_dict(pptx_path: Path, issues_only: bool = False) -> InventoryDict:
    ...
    for slide_key, slide_data in inventory.items():
        ...
        for key, value in slide_data.items():
            if key == "notes":
                dict_inventory[slide_key][key] = value
            else:
                dict_inventory[slide_key][key] = value.to_dict()
...
def save_inventory(inventory: InventoryData, output_path: Path) -> None:
    ...
    for slide_key, slide_data in inventory.items():
        ...
        for key, value in slide_data.items():
            if key == "notes":
                json_inventory[slide_key][key] = value
            else:
                json_inventory[slide_key][key] = value.to_dict()
