...
def detect_frame_overflow(inventory: InventoryData) -> Dict[str, Dict[str, float]]:
    ...
    for slide_key, shapes_dict in inventory.items():
        for shape_key, shape_data in shapes_dict.items():
            # Skip non-shape entries like 'notes'
            if shape_key == "notes":
                continue
...
def apply_replacements(pptx_file: str, json_file: str, output_file: str):
    ...
    for slide_key, slide_data in inventory.items():
        ...
        # Process notes if present in replacements
        replacement_slide_data = replacements.get(slide_key, {})
        if "notes" in replacement_slide_data:
            try:
                if not slide.has_notes_slide:
                    # Create notes slide if it doesn't exist
                    slide.notes_slide
                if slide.has_notes_slide:
                    slide.notes_slide.notes_text_frame.text = replacement_slide_data["notes"]
            except Exception as e:
                print(f"Warning: Failed to update notes for {slide_key}: {e}")

        # Process each shape from inventory
        for shape_key, value in slide_data.items():
            if shape_key == "notes":
                continue
    ...
    if (overflow_errors or warnings) and os.environ.get("SKIP_OVERFLOW_VALIDATION") != "1":
