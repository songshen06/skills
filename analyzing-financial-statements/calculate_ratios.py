...
class FinancialRatioCalculator:
    ...
    def interpret_ratio(self, ratio_name: str, value: float, lang: str = "en") -> str:
        """Provide interpretation for a specific ratio."""
        interpretations = {
            "current_ratio": lambda v: (
                ("Strong liquidity" if lang == "en" else "流动性强")
                if v > 2
                else ("Adequate liquidity" if lang == "en" else "流动性充足")
                if v > 1.5
                else ("Potential liquidity concerns" if lang == "en" else "潜在流动性担忧")
                if v > 1
                else ("Liquidity issues" if lang == "en" else "流动性问题")
            ),
            "debt_to_equity": lambda v: (
                ("Low leverage" if lang == "en" else "低杠杆")
                if v < 0.5
                else ("Moderate leverage" if lang == "en" else "适度杠杆")
                if v < 1
                else ("High leverage" if lang == "en" else "高杠杆")
                if v < 2
                else ("Very high leverage" if lang == "en" else "极高杠杆")
            ),
            "roe": lambda v: (
                ("Excellent returns" if lang == "en" else "极佳回报")
                if v > 0.20
                else ("Good returns" if lang == "en" else "良好回报")
                if v > 0.15
                else ("Average returns" if lang == "en" else "平均回报")
                if v > 0.10
                else ("Below average returns" if lang == "en" else "低于平均回报")
                if v > 0
                else ("Negative returns" if lang == "en" else "负回报")
            ),
            "pe_ratio": lambda v: (
                ("Potentially undervalued" if lang == "en" else "可能被低估")
                if 0 < v < 15
                else ("Fair value" if lang == "en" else "估值合理")
                if 15 <= v < 25
                else ("Growth premium" if lang == "en" else "成长溢价")
                if 25 <= v < 40
                else ("High valuation" if lang == "en" else "高估值")
                if v >= 40
                else ("N/A (negative earnings)" if lang == "en" else "不可用 (负收益)")
                if v <= 0
                else "N/A"
            ),
        }
        ...
        return "No interpretation available" if lang == "en" else "无可用解读"
...
def calculate_ratios_from_data(financial_data: dict[str, Any], lang: str = "en") -> dict[str, Any]:
    """
    ...
    for category, category_ratios in ratios.items():
        ...
        for ratio_name, value in category_ratios.items():
            interpretations[category][ratio_name] = {
                ...
                "interpretation": calculator.interpret_ratio(ratio_name, value, lang=lang),
            }

    return {
        "ratios": ratios,
        "interpretations": interpretations,
        "summary": generate_summary(ratios, lang=lang),
    }


def generate_summary(ratios: dict[str, Any], lang: str = "en") -> str:
    ...
    if prof.get("roe", 0) > 0:
        if lang == "zh":
            summary_parts.append(
                f"ROE 为 {prof['roe'] * 100:.1f}%，表明股东回报{'强劲' if prof['roe'] > 0.15 else '适中'}。"
            )
        else:
            summary_parts.append(
    ...
    if liq.get("current_ratio", 0) > 0:
        if lang == "zh":
            summary_parts.append(
                f"流动比率为 {liq['current_ratio']:.2f}，表明流动性状况{'良好' if liq['current_ratio'] > 1.5 else '可能存在担忧'}。"
            )
        else:
            summary_parts.append(
    ...
    if lev.get("debt_to_equity", 0) >= 0:
        if lang == "zh":
            summary_parts.append(
                f"产权比率为 {lev['debt_to_equity']:.2f}，表明杠杆水平{'保守' if lev['debt_to_equity'] < 0.5 else '适中' if lev['debt_to_equity'] < 1 else '较高'}。"
            )
        else:
            summary_parts.append(
    ...
    if val.get("pe_ratio", 0) > 0:
        if lang == "zh":
            summary_parts.append(
                f"市盈率为 {val['pe_ratio']:.1f}，表明股价{'被低估' if val['pe_ratio'] < 15 else '合理' if val['pe_ratio'] < 25 else '存在溢价'}。"
            )
        else:
            summary_parts.append(
    ...
    if not summary_parts:
        return "数据不足，无法生成摘要。" if lang == "zh" else "Insufficient data for summary."
